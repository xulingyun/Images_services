...
//			Query sq = s.createSQLQuery("select * from Data where status=0 and area="+area);
			Query sq = s.createQuery("select count(*) from Data where status=? and area=?").setInteger(0, 0).setInteger(1, Integer.parseInt(area));
			System.out.println(((Number)sq.uniqueResult()).intValue());
